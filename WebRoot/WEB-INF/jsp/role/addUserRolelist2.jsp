	...
	function checkThis(){
	 ...
	 var $conetnt = $(".dialogContent", $.pdialog._current);
		 $conetnt.find('div.gridTbody').find('input[type=checkbox]:checked').each(function(){
		  ans.push($(this).val());
		  ans2.push($(this).parent().find('input[type=hidden]').val());
		 });
		 var selectId = ans.join(','); 
		  var selectName = ans2.join(','); 
		 $('#roleIds').val(selectId);
		  $('#roleNames').val(selectName);
		  $.pdialog.closeCurrent();
	}
...
<div id='addUserRoleDiv'>
	<form id="pagerForm" method="post"
		action="/money/role!beforeRoleInUser2.do">
		...
		<input
			...
			type="hidden" name="orderDirection" value="${param.orderDirection}" />
				<input
			type="text" name="roleType" id="roleType" value="${roleType}" />
				<input
			type="text" name="roleKey" id="roleKey" value="${roleKey}" />
	</form>
	<div class="pageHeader">
		<form onsubmit="return refreshSelf(this);"
			...
			<div class="searchBar">
				...
				<div class="subBar">
					<ul> 
						<li><a class="button" href="javascript:return false;" onclick="checkThis()"><span>选择</span>
						</a>
						</li>
				...
				<s:iterator value="list" status="stu">
					<tr target="roleId" rel="<s:property value="roleId" />">
						<td><input name="ids" value="<s:property value="roleId" />"
							type="checkbox">
							<input name="rnm" id="rnm" value="<s:property value="roleName" />"
							type="hidden">
						</td>
