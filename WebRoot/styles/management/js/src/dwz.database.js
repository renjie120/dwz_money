...
(function($){
	...
	$.extend({
		...
		returnValue: function(formid,thisformId){ 
			var $form = $('#'+formid); 
			var $thisform = $('#'+thisformId); 
			var jsonObj = $thisform.serializeArray();  
			$form.find(":input").each(function(){
				var $input = $(this), inputName = $input.attr("name");  
				jQuery.each(jsonObj, function(i, field){ 
			...
			return this.each(function(){
				...
				$this.click(function(event){ 
					_lookup = $.extend(_lookup, {
						...
						pks: $this.attr("lookupToPks") || false,
						$target: $this,
		...
		suggest: function(){
			...
			return this.each(function(){
				...
				function _show(event){
					...
					var url  ='';
					if($input.attr('keyId')){
						var target = $(event.target).parents("form").find('#'+$input.attr('keyId')).val();
						url = unescape($input.attr("suggestUrl")).replace('$',target);
					}
					else
						url = unescape($input.attr("suggestUrl")).replaceTmById($(event.target).parents(".unitBox:first"));
