 ...
 * @return {Boolean}
 */
function myCallback(form, callback) {
	var $form = $(form);
	if (!$form.valid()) {
		return false;
	}
	$.ajax({
		...
		cache : false,
		success : callback || DWZ.ajaxDone,
		error : callback || DWZ.ajaxError
			// 这里和上面的validateCallback不同，不知道为什么总是jquery在正确的情况也到error里面
		});
	return false;
}

/**
 * 弹出框体保存之后关闭当前页面并刷新父页面
 * 
 * @param {}
 *            txt
 */
