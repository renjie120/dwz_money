...
public class AddMoneyDetailAction extends BaseAction {
	...
	// 业务接口对象.
	AddMoneyDetailManager pMgr = bf
			.getManager(BeanManagerKey.addmoneydetailManager);
	...
	public String doAdd() {
		try {
			...
			AddMoneyDetailImpl addmoneydetailImpl = new AddMoneyDetailImpl(
					iuserId, addType, addMoney, insuredFileId, addTime,
					createUser, createTime, updateUser, updateTime);
			pMgr.createAddMoneyDetail(addmoneydetailImpl);
			userMgr.addMoney(iuserId, addType, Double.parseDouble(addMoney));
			insertLog(logMgr, "添加充值记录明细", "/doAdd", "", "",
					JSON.toJSONString(addmoneydetailImpl));
	...
	public String doDelete() {
		...
		insertLog(logMgr, "删除充值记录明细", "/doDelete", "", "",
	...
	public String doUpdate() {
		try {
			...
			AddMoneyDetailImpl addmoneydetailImpl = new AddMoneyDetailImpl(sno,
					iuserId, addType, addMoney, insuredFileId, addTime,
					createUser, createTime, updateUser, updateTime);
	...
	public enum ExportFiled {
		SNO("流水号"), IUSERID("投保用户号"), ADDTYPE("充值字段"), ADDMONEY("充值金额 "), INSUREDFILEID(
				"投保单号 "), ADDTIME("充值时间"), CREATEUSER("创建用户"), CREATETIME(
				"创建时间");
		private String str;
	...
	public enum ImportFiled {
		IUSERID("投保用户号"), ADDTYPE("充值字段"), ADDMONEY("充值金额 "), INSUREDFILEID(
				"投保单号 ");
		private String str;
	...
	public String beforeQuery() {
		...
		request.setAttribute("addtype_list",
				select_addmoneytype.getSelectAbles());
		return "query";
	}
	...
	public String export() {
		...
		response.addHeader("Content-Disposition",
				"attachment;filename=AddMoneyDetailList.xls");
		...
		Collection<AddMoneyDetail> addmoneydetailList = pMgr
				.searchAddMoneyDetail(criterias, realOrderField(), startIndex,
						numPerPage);
			...
			for (ExportFiled filed : ExportFiled.values()) {
				...
				case INSUREDFILEID:
					e.setCell(filed.ordinal(),
							addmoneydetail.getInsuredFileId());
					break;
				case ADDTIME:
				...
				case CREATEUSER:
					e.setCell(
							filed.ordinal(),
							CacheUtil.getSystemUserName(""
									+ addmoneydetail.getCreateUser()));
					break;
				case CREATETIME:
	...
	public String query() {
		...
		Collection<AddMoneyDetail> moneyList = pMgr.searchAddMoneyDetail(
	...
	private Map<AddMoneyDetailSearchFields, Object> getCriterias() {
		...
		if (getIuserId() != null && !"".equals(getIuserId())
				&& !"-1".equals(getIuserId()) && !"-2".equals(getIuserId()))
			criterias.put(AddMoneyDetailSearchFields.IUSERID, getIuserId());
		if (getInsuredFileId() != null && !"".equals(getInsuredFileId())
				&& !"-1".equals(getInsuredFileId())
				&& !"-2".equals(getInsuredFileId()))
			criterias.put(AddMoneyDetailSearchFields.INSUREDFILEID,
					getInsuredFileId());

		// 下面是高级查询的时候添加的条件
		// 添加投保用户号的查询条件
		addIuserIdCondition(criterias, getCondition_iuserId(),
				getQuery_iuserId());
		// 添加充值字段的查询条件
		addAddTypeCondition(criterias, getCondition_addType(),
				getQuery_addType());
		// 添加充值金额 的查询条件
		addAddMoneyCondition(criterias, getCondition_addMoney(),
				getQuery_addMoney());
		// 添加投保单号 的查询条件
		addInsuredFileIdCondition(criterias, getCondition_insuredFileId(),
				getQuery_insuredFileId());
		// 添加充值时间的查询条件
		addAddTimeCondition(criterias, getCondition_addTime(),
				getQuery_addTime());
		// 添加创建用户的查询条件
		addCreateUserCondition(criterias, getCondition_createUser(),
				getQuery_createUser());
		// 添加创建时间的查询条件
		addCreateTimeCondition(criterias, getCondition_createTime(),
				getQuery_createTime());
		// 添加更新用户的查询条件
		addUpdateUserCondition(criterias, getCondition_updateUser(),
				getQuery_updateUser());
		// 添加更新时间的查询条件
		addUpdateTimeCondition(criterias, getCondition_updateTime(),
				getQuery_updateTime());
		return criterias;
	}
	 ...
	 * 添加查询投保用户号的查询条件.
	 */
	public void addIuserIdCondition(
			Map<AddMoneyDetailSearchFields, Object> criterias,
		...
		if (Constants.COM_EQUALS.equals(condition)) {
			...
			criterias.put(AddMoneyDetailSearchFields.IUSERID_COM_NOT_EQUALS,
					value);
		}
	}
	 ...
	 * 添加查询充值字段的查询条件.
	 */
	public void addAddTypeCondition(
			Map<AddMoneyDetailSearchFields, Object> criterias,
			String condition, String value) {
		if ("-1".equals(condition))
			return;
		if (Constants.COM_EQUALS.equals(condition)) {
			...
			criterias.put(AddMoneyDetailSearchFields.ADDTYPE_COM_NOT_EQUALS,
					value);
		}
	}
	 ...
	 * 添加查询充值金额 的查询条件.
	 */
	public void addAddMoneyCondition(
			Map<AddMoneyDetailSearchFields, Object> criterias,
			String condition, String value) {
		if ("-1".equals(condition))
			return;
		if (Constants.COM_EQUALS.equals(condition)) {
			criterias
					.put(AddMoneyDetailSearchFields.ADDMONEY_COM_EQUALS, value);
		} else if (Constants.COM_NOT_EQUALS.equals(condition)) {
			criterias.put(AddMoneyDetailSearchFields.ADDMONEY_COM_NOT_EQUALS,
					value);
		}
	}
	 ...
	 * 添加查询投保单号 的查询条件.
	 */
	public void addInsuredFileIdCondition(
			Map<AddMoneyDetailSearchFields, Object> criterias,
			String condition, String value) {
		if ("-1".equals(condition))
			return;
		if (Constants.COM_EQUALS.equals(condition)) {
			criterias.put(AddMoneyDetailSearchFields.INSUREDFILEID_COM_EQUALS,
					value);
		} else if (Constants.COM_NOT_EQUALS.equals(condition)) {
			criterias.put(
					AddMoneyDetailSearchFields.INSUREDFILEID_COM_NOT_EQUALS,
					value);
		}
	}
	 ...
	 * 添加查询充值时间的查询条件.
	 */
	public void addAddTimeCondition(
			Map<AddMoneyDetailSearchFields, Object> criterias,
			String condition, String value) {
		if ("-1".equals(condition))
			return;
		if (Constants.COM_EQUALS.equals(condition)) {
			...
			criterias.put(AddMoneyDetailSearchFields.ADDTIME_COM_NOT_EQUALS,
					value);
		}
	}
	...
	public void addCreateUserCondition(
		...
		if ("-1".equals(condition))
			return;
		if (Constants.COM_EQUALS.equals(condition)) {
			criterias.put(AddMoneyDetailSearchFields.CREATEUSER_COM_EQUALS,
					value);
		} else if (Constants.COM_NOT_EQUALS.equals(condition)) {
			criterias.put(AddMoneyDetailSearchFields.CREATEUSER_COM_NOT_EQUALS,
					value);
		}
	}
	...
	public void addCreateTimeCondition(
		...
		if ("-1".equals(condition))
			return;
		if (Constants.COM_EQUALS.equals(condition)) {
			criterias.put(AddMoneyDetailSearchFields.CREATETIME_COM_EQUALS,
					value);
		} else if (Constants.COM_NOT_EQUALS.equals(condition)) {
			criterias.put(AddMoneyDetailSearchFields.CREATETIME_COM_NOT_EQUALS,
					value);
		}
	}
	...
	public void addUpdateUserCondition(
		...
		if ("-1".equals(condition))
			return;
		if (Constants.COM_EQUALS.equals(condition)) {
			criterias.put(AddMoneyDetailSearchFields.UPDATEUSER_COM_EQUALS,
					value);
		} else if (Constants.COM_NOT_EQUALS.equals(condition)) {
			criterias.put(AddMoneyDetailSearchFields.UPDATEUSER_COM_NOT_EQUALS,
					value);
		}
	}
	...
	public void addUpdateTimeCondition(
		...
		if ("-1".equals(condition))
			return;
		if (Constants.COM_EQUALS.equals(condition)) {
			criterias.put(AddMoneyDetailSearchFields.UPDATETIME_COM_EQUALS,
					value);
		} else if (Constants.COM_NOT_EQUALS.equals(condition)) {
			criterias.put(AddMoneyDetailSearchFields.UPDATETIME_COM_NOT_EQUALS,
					value);
		}
	}
