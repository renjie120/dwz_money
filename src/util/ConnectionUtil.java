 ...
 * <p>
 * This class shows two ways of setting database connection properties (JDBC
 * driver, database URL, database user name, password). The first method fills
 * properties according to the given parameters, while the second method loads
 * these settings from the user configuration file (.jchem file).
 * <p>
 ...
 * </ul>
 * <p>
 * Examples of connection settings can be found in the <a
 * href="http://www.chemaxon.com/jchem/doc/admin/JChemBaseFAQ.html#dburl"
 * >JChemBase FAQ</a>.
 * 
...
public final class ConnectionUtil {
	 ...
	 */
	public static ConnectionHandler getConnectionHandler(String driverClass,
	...
	/**
	 * Returns a connection handler using properties defined in user settings
	 * (the .jchem configuration file).
	 * 
	 * @return initialized connection handler
	 * @throws IOException
	 *             if JDBC driver or database URL is missing in the user
	 *             settings
	 */
	public static ConnectionHandler getDefaultConnectionHandler()
			throws IOException {
		ConnectionHandler connHandler = new ConnectionHandler();
	...
	public static ConnectionHandler getMyDefaultConnectionHandler()
			throws IOException {
		// Properties props = new SettingsHandler().getSettings();
		Properties props = new Properties();

		InputStream in = new BufferedInputStream(new FileInputStream("e:\\jchem.properties"));
		props.load(in);
		
		String host = props.getProperty("connection.host");
		String username = props.getProperty("connection.username");
		String database = props.getProperty("connection.database");
		String password = props.getProperty("connection.password"); 
		ConnectionHandler connHandler = ConnectionHandler.getInstance(3, host, 3306, database, username, password);
...
//		}
	...
	/**
	 * Saves the properties of the given connection handler to user settings
	 * (the .jchem configuration file).
	 * 
	 * @param connHandler
	 *            connection handler
	 * @throws IOException
	 *             if the properties cannot be saved
	 */
	...
	/**
	 * Connects to the database specified in the user settings (the .jchem
	 * configuration file).
	 * 
	...
	public static ConnectionHandler connectToDB() throws IOException {
		try {
			ConnectionHandler connHandler = ConnectionUtil
					.getMyDefaultConnectionHandler();
			connHandler.connectToDatabase();
			System.out.println("Connection estabilished to "
					+ connHandler.getUrl());
			return connHandler;
		} catch (Exception e) {
