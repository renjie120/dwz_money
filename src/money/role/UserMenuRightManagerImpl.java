 ...
 * http://www.iteye.com
 */ 
public class UserMenuRightManagerImpl extends AbstractBusinessObjectManager implements
	...
	public Collection<UserMenuRight> searchUserMenuRight(Map<UserMenuRightSearchFields, Object> criterias,
		...
		String hql = quertParas[0].toString();
		Collection<UserMenuRightVO> voList = this.usermenurightdao.findByQuery(hql,
	...
	private Object[] createQuery(boolean useCount,
		...
		sb.append(
				useCount ? "select count(distinct usermenuright) "
						: "select distinct usermenuright ").append("from UserMenuRightVO as usermenuright ");
	...
	public void removeUserMenuRights(String ids) {
		String[] idArr = ids.split(",");
		for (String s : idArr) {
			UserMenuRightVO vo = this.usermenurightdao.findByPrimaryKey(Integer.parseInt(s));
			this.usermenurightdao.delete(vo);
		}
	}
	...
	public UserMenuRight getUserMenuRight(int id) {
		Collection<UserMenuRightVO> usermenurights = this.usermenurightdao.findRecordByUserId(id);

		if (usermenurights == null || usermenurights.size() < 1)
			return null;

		UserMenuRightVO usermenuright = usermenurights.toArray(new UserMenuRightVO[usermenurights.size()])[0];

		return new UserMenuRightImpl(usermenuright);
	}
