...
public class StockManagerAction extends BaseAction {
	...
	//业务接口对象.
	StockManagerManager pMgr = bf.getManager(BeanManagerKey.stockmanagerManager);
	//业务实体对象
	...
	public String doAdd() {
		try {
			StockManagerImpl stockmanagerImpl = new StockManagerImpl(stockNo ,stockName ,dealDate ,price ,dealNumber ,fee ,dealType );
	...
	public String doUpdate() {
		try {
			StockManagerImpl stockmanagerImpl = new StockManagerImpl( sno , stockNo , stockName , dealDate , price , dealNumber , fee , dealType );
	...
	public enum ExportFiled {
		  SNO("流水号"),  STOCKNO("股票号码"),  STOCKNAME("股票名称 "),  DEALDATE("交易时间"),  PRICE("交易价格"),  DEALNUMBER("交易份额"),  FEE("交易费率"),  DEALTYPE("交易类型");
		private String str;
	...
	public String export() {
		...
		response.addHeader("Content-Disposition","attachment;filename=StockManagerList.xls");
		...
		Collection<StockManager> stockmanagerList = pMgr.searchStockManager(criterias, realOrderField(),
	...
	public String query() {
		...
		Collection<StockManager> moneyList = pMgr.searchStockManager(criterias, realOrderField(),
				startIndex, numPerPage);
