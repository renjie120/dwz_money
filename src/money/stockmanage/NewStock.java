...
public class NewStock {
	...
	public NewStock(double stockValue, double count, String stockName,
		...
		stocklist.add(Double.valueOf(stockValue));
		LinkedList countList = new LinkedList();
		countList.add(Double.valueOf(count));
		...
		buyCostList.add(Double.valueOf(buyCost));

		this.stock.setCount(count);
		this.stock.setBuyCost(buyCost);
		this.stock.setStockValue(stockValue);
		this.stock.setStockName(stockName);

		this.stockValues = stocklist;
		this.counts = countList;
		this.buyCosts = buyCostList;
	}
	...
	public void addStock(StockVO vo) {
		...
		this.stock.setBuyCost(vo.getBuyCost());
		this.stock.setCount(vo.getCount());
		this.stock.setStockValue(vo.getStockValue());
	}
	...
	private double getAllMoney() {
		...
		while (cit.hasNext()) {
			...
			ans = CommonUtil.add(ans, CommonUtil.add(CommonUtil.multiply(ct, vt), bt));
		}
		return ans;
	}
	...
	private double getDeadLineValue(double sellCost) {
		return CommonUtil.divide(CommonUtil.multiply(getAllMoney(), 1D + sellCost),
				getCount(), 5, RoundingMode.HALF_EVEN);
	}
